(* The file presents various inversion lemmas for the program level. *)

From mathcomp Require Import ssreflect ssrbool eqtype ssrnat seq.
From Coq Require Import ssrfun Classical Utf8.
Require Export AutosubstSsr ARS logical_uniq logical_sr program_subst.

Set Implicit Arguments.
Unset Strict Implicit.
Unset Printing Implicit Defensive.

Lemma program_lam0_invX Γ Δ A1 A2 B C m s1 s2 t :
  Γ ; Δ ⊢ Lam0 A1 m s1 : C ->
  C === Pi0 A2 B s2 ->
  (A2 :: Γ) ⊢ B : Sort t ->
  (A2 :: Γ) ; _: Δ ⊢ m : B.
...
Lemma program_lam1_invX Γ Δ A1 A2 B C m s1 s2 t :
  Γ ; Δ ⊢ Lam1 A1 m s1 : C ->
  C === Pi1 A2 B s2 ->
  (A2 :: Γ) ⊢ B : Sort t ->
...
Proof with eauto.
  move=>ty.
  have[t/logical_pi0_inv[r[tyB _]]]:=program_valid ty.
...
Proof with eauto.
  move=>ty.
  have[t/logical_pi1_inv[r[tyB _]]]:=program_valid ty.
  apply: program_lam1_invX...
Qed.

Lemma program_pair0_invX Γ Δ A B m n s r t C :
  Γ ; Δ ⊢ Pair0 m n s : C ->
  C === Sig0 A B r ->
  Γ ⊢ Sig0 A B r : Sort t ->
  s = r /\ Γ ; Δ ⊢ m : A /\ Γ ⊢ n : B.[m/].
...
Lemma program_pair1_invX Γ Δ A B m n s r t C :
  Γ ; Δ ⊢ Pair1 m n s : C ->
  C === Sig1 A B r ->
  Γ ⊢ Sig1 A B r : Sort t ->
  exists Δ1 Δ2,
...
Lemma program_apair_invX Γ Δ A B m n s r t C :
  Γ ; Δ ⊢ APair m n s : C ->
  C === With A B r ->
  Γ ⊢ With A B r : Sort t ->
  s = r /\ Γ ; Δ ⊢ m : A /\ Γ ; Δ ⊢ n : B.
...
Lemma program_apair_inv Γ Δ A B m n s r :
  Γ ; Δ ⊢ APair m n s : With A B r ->
  s = r /\ Γ ; Δ ⊢ m : A /\ Γ ; Δ ⊢ n : B.
Proof with eauto.
  move=>ty.
  have[t tyW]:=program_valid ty.
  apply: program_apair_invX...
Qed.
